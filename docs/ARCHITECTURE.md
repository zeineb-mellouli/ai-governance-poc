...
`policies/policies.yaml` governs two halves of the same problem. This document
describes the runtime half; the build-time half is where the rules are applied
...
```
                    policies/policies.yaml
                     (one source of truth)
                       /              \
              rules.md                ChromaDB
        the constitution              policy retrieval
                 |                          |
     BUILD TIME: spec-driven         RUNTIME: this pipeline
     generation writes the code      audits the code that was written
```
...
`specs/001-*/` (spec, plan, tasks, data-model, research), and the
`speckit.*.agent.md` prompt definitions. Their constitutions cite the policy ids
directly, so a generated repository can be traced back to the rule that shaped
it.

**Runtime.** The same policies are embedded into ChromaDB and applied by the
Auditor to code that already exists, whoever or whatever wrote it.

The point of the split is that a rule cannot drift between guidance and
enforcement: change `policies.yaml` and both halves are regenerated from it.
`agents/repository_agent.py` excludes `specs/`, `.specify/` and `.github/agents/`
from the audit for the same reason, since governance applies to the pipeline that
