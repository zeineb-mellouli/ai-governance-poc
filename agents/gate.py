...
Two independent thresholds, both off by default:

  fail_on        a NON_COMPLIANT finding at this severity or above
  fail_on_error  the audit did not complete cleanly

There was a fail_on_grade too. It went when the PASS/NEEDS_WORK/FAIL grade did:
gating on a composite verdict meant the build's outcome moved whenever an
arbitrary rate band moved. Severity counts are concrete and are what a reviewer
argues about anyway.

The second exists because the first is only meaningful if the audit
