...
Inspect all filenames, folder names, and data file column headers for: (1) CamelCase on dataset/file names -- flag snake_case, all-lowercase, or space-separated file names as NON_COMPLIANT (e.g. ethanol_market_rate.csv or "Ethanol Market Rate.csv" are violations; EthanolMarketRate_2024-07-01.csv is compliant); (2) date suffixes must be yyyy-MM-dd format -- flag yyyyMMdd (no separators, e.g. 20240701) or dd-MM-yyyy or any other variant as NON_COMPLIANT; the correct form is 2024-07-01 with hyphens; (3) folder names must use <Project>_<Feature> as a single folder name with an underscore separator (e.g. Polymers_MarketRate, Treasury_Pipeline) -- do NOT flag this pattern as non-compliant; a two-level nested path like Polymers/MarketRate is also acceptable but the single-folder underscore pattern is preferred; (4) vague or versioned names anywhere in the tree: Untitled, final, copy, v2, ACTUAL, temp, test123 -- flag as NON_COMPLIANT; (5) names ending with underscore, containing consecutive underscores, or containing spaces -- flag as NON_COMPLIANT; (6) names starting with a digit or special character -- flag as NON_COMPLIANT; (7) data file column headers not in snake_case, or plural column names (e.g. customers, products) -- flag as NON_COMPLIANT; (8) missing README.md at the repository root -- flag as NON_COMPLIANT. Infrastructure and CI/CD files with system-mandated names that cannot be freely renamed (azure-pipelines.yml, .github/workflows/*.yml, Dockerfile, Makefile, .gitignore, requirements.txt, setup.cfg, pyproject.toml, conftest.py, __init__.py, README.md) are exempt from the CamelCase rule -- do not flag them as NON_COMPLIANT for their own filename. The repository root folder name is governed exclusively by REPO-9; do not evaluate it under this policy.
