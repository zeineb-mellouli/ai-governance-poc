...
Search all code and notebook cells for patterns resembling API keys, connection strings with embedded passwords, or tokens (e.g. postgresql://user:password@host, api_key = "sk-...", hardcoded AWS credentials). Environment variable usage (os.environ, os.getenv) or a secrets manager reference is compliant. A literal credential value assigned directly in code is NON_COMPLIANT regardless of repo visibility. Return NOT_APPLICABLE for template and example environment files (.env.example, .env.template, .env.sample, or any file whose name signals it is a template): placeholder syntax such as <PLACEHOLDER>, <secret-never-commit>, your-value-here, REPLACE_ME, or an empty assignment (KEY=) documents what secrets are required without embedding actual values -- these are compliant by design and must not be flagged. Only flag a real, non-placeholder credential value that could actually authenticate a system.
...
All files and folders must follow the organisation's naming convention. Dataset and file names are written in CamelCase. Folder structure uses the pattern <Project>_<Feature> as a single folder name using an underscore separator (e.g. Polymers_MarketRate). File names follow <DatasetName>_<yyyy-MM-dd> (e.g. EthanolMarketRate_2024-07-01.csv). Rules that apply to all names: begin with a letter; do not end with an underscore; only letters, numbers, and underscores (plus hyphens in the date portion only); no consecutive underscores; no spaces; max 126 characters; avoid abbreviations unless commonly understood. Column names in data files (CSV, Parquet) use snake_case and are always singular. A README describing the project's purpose and structure is required at the repository root.

**Applies when:** Every repo.

**How to evaluate:**

Apply these rules in order. Flag NON_COMPLIANT only for a clear, specific violation. When in doubt, return COMPLIANT.
FILES: Dataset and pipeline file names must be CamelCase, optionally followed by an underscore and a yyyy-MM-dd date suffix. Compliant: EthanolMarketRate_2024-07-01.csv, IngestData.py. Violations: ethanol_market_rate.csv, ingest_data.py, 20240701 date suffix (no hyphens), space in name, name starting with a digit.
FOLDERS: Pipeline and feature folders use <Project>_<Feature> with a single underscore (e.g. Risk_Pipeline, Treasury_SQL, Collateral_Pipeline). This IS the correct pattern -- never flag it. Medallion layer folders (bronze, silver, gold) are exempt by convention. The repository root folder is governed by REPO-9 only.
VAGUE NAMES: Flag Untitled, final, copy, v2, ACTUAL, temp as NON_COMPLIANT anywhere in the file tree.
CSV/PARQUET COLUMNS: Headers must be snake_case and singular. Violations: customerID (camelCase), customers (plural).
README: A README.md must exist at the repository root; flag missing.
EXEMPT from CamelCase: azure-pipelines.yml, .github/workflows/*.yml, Dockerfile, Makefile, .gitignore, requirements.txt, setup.cfg, pyproject.toml, conftest.py, __init__.py, README.md.
RENAME SUGGESTIONS: Always suggest the CamelCase_yyyy-MM-dd.csv target. Never suggest snake_case, kebab-case, or all-lowercase -- those are violations themselves.
...
Search for validation logic appearing after data is loaded and before it is used further. Any of the following counts as compliant: (a) Databricks Lakeflow expectation decorators (@dp.expect_all, @dp.expect_all_or_drop, @dp.expect_all_or_fail) or a CONSTRAINT ... EXPECT clause; (b) a Great Expectations, pandera, or PyDeequ validation call; (c) explicit programmatic checks (assert / raise / filter) validating null counts, duplicate counts, value ranges, or train/test leakage. If data is loaded and used with no such checks anywhere in the file, flag NON_COMPLIANT. Return NOT_APPLICABLE (do not flag) when the file is any of the following: (1) a raw data file (CSV, Parquet, JSON data, or similar) containing no executable code -- data quality validation is the responsibility of the code that consumes it, not the file itself; (2) a SQL DDL file whose content is exclusively schema definitions (CREATE TABLE, CREATE VIEW, CREATE INDEX, ALTER TABLE, DROP TABLE) with no data loading or INSERT/SELECT statements -- DDL defines structure, it does not load or consume data; (3) a markdown or documentation file (README.md, .md, .rst, .txt) -- documentation files contain no executable data loading code; (4) an empty or near-empty Python package marker file (__init__.py with no substantive code body) -- package markers are not data processing modules.
...
Check saved notebook cell outputs and any print()/log statements for dataframes or values containing identifier-like columns with real-looking, non-masked, non-synthetic values visible. Using .head() on a masked or synthetic sample, redacting before display, or clearing notebook outputs before commit is compliant. Flag NON_COMPLIANT if raw identifier values are visible in a saved output or a print/log statement. Return NOT_APPLICABLE for test files that contain PII-like strings only as regex patterns or assertion strings verifying that those patterns do NOT appear in logs or outputs (e.g. a test asserting no email address appears in a log file). Such files are PII guardrails, not PII exposures.
...
Check for: (1) a requirements.txt, environment.yml, or pyproject.toml with pinned, not floating/unversioned, package versions; (2) random seeds explicitly set for any stochastic step (np.random.seed, random.seed, random_state= in sklearn, or framework equivalents); (3) raw data read-only or copied rather than transformed in place. Flag NON_COMPLIANT if dependencies are unpinned, no random seed is set anywhere in a training/sampling step, or raw source files are directly overwritten by processing code. When evaluating a Python or notebook file in isolation: only check for random seeds within that file; do not flag it NON_COMPLIANT for a missing requirements file -- the dependency-pinning check applies when evaluating requirements.txt / environment.yml / pyproject.toml directly. If the file performs no stochastic operations (no imports of random, numpy.random, sklearn, torch, or similar stochastic libraries; no model training or sampling calls), return NOT_APPLICABLE -- reproducibility seeding only applies where randomness is actually introduced.
...
Check for: (1) CI/CD YAML files referencing branches -- flag any branch name that is not master, develop, or matching user-story/\d+ as NON_COMPLIANT (e.g. hotfix-thing, my-branch, test, feature/xyz are violations); (2) a PR template or CONTRIBUTING.md -- if present, check it enforces at least one reviewer and references a user story; (3) any commit message samples or git log snippets -- flag messages without a conventional prefix (feat:, fix:, chore:, docs:, refactor:) or messages that bundle multiple unrelated changes in one commit; (4) if a branch protection config file exists, check that master and develop require reviews before merge. Flag NON_COMPLIANT for any deviation from the three-tier branch model or missing review requirement. When reviewing branch names in documentation files (README.md, CONTRIBUTING.md), treat {id} or {number} as a valid numeric placeholder equivalent to \d+ -- do not flag it as a non-compliant branch name. Only flag concrete branch names found in CI/CD YAML trigger/pr/branches sections or in git log output. SCOPE: only evaluate files that are CI/CD pipeline configurations (azure-pipelines.yml, .github/workflows/*.yml), PR templates (PULL_REQUEST_TEMPLATE.md), or developer contribution guides (CONTRIBUTING.md). Agent instruction files, spec-kit workflow files, and general design or specification documents (e.g. .github/agents/*.md, *.agent.md, specs/**/*.md, quickstart.md, data-model.md, tasks.md) are NOT governance control files and must return NOT_APPLICABLE for this policy.
...
Only evaluate .sql files and SQL strings embedded in .py or .ipynb files. Do NOT evaluate README.md or other markdown/documentation files under this policy -- they describe SQL objects but are not SQL code. Scan SQL code for: (1) table or view names not in PascalCase -- flag all-lowercase or snake_case names (e.g. ethanol_market_rate, tbl_ethanol) as NON_COMPLIANT; (2) any Hungarian notation prefix: tbl_, vw_, sp_, fn_, udf_ -- flag as NON_COMPLIANT regardless of the rest of the name; (3) data model tables (used in star-schema joins with dimension/fact relationships) missing the Dim or Fact suffix -- flag as NON_COMPLIANT. The Dim/Fact suffix must appear at the END of the name (e.g. EthanolMarketRateFact, CustomerDim are correct; FactEthanolMarketRate or DimCustomer are wrong prefix forms and should be flagged). A table already ending in Fact or Dim is compliant and must NEVER be renamed to add a second suffix (e.g. LiquidityForecastFact is already compliant -- do not suggest LiquidityForecastFactFact); (4) stored procedure OBJECT NAMES (the identifier after CREATE PROCEDURE) using a forbidden verb like Create, Creating, Delete, Drop -- flag as NON_COMPLIANT. This rule applies ONLY to the SQL object name defined inside the file, never to the .sql filename itself (a file named CreateCustomerFact.sql is a normal DDL file convention and must NOT be flagged under this rule); (5) schema names not in the approved list (Staging, Production, MetaData, Logging, Config, Reporting, PowerBI, DataMart) or that use underscores or exceed 30 characters -- flag as NON_COMPLIANT; (6) any table whose name exactly matches one of its own column names -- flag as NON_COMPLIANT. Return NOT_APPLICABLE if no SQL object definitions are present.
